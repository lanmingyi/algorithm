...
def _get_image_blob(im):
    ...
    im_orig -= cfg.FLAGS2["pixel_means"]  # 去均值
...
def im_detect(sess, net, im):
    ...
    if cfg.FLAGS.test_bbox_reg:  # 默认true
...
def test_net(sess, net, imdb, weights_filename, max_per_image=100, thresh=0.05):
    ...
    for i in range(num_images):
        ...
        scores, boxes = im_detect(sess, net, im)  # 调用im_detect对图像检测，返回类别和坐标信息
        ...
        for j in range(1, imdb.num_classes):
            inds = np.where(scores[:, j] > thresh)[0]  # 返回在类别j上得分大于阈值的横坐标
            cls_scores = scores[inds, j]  # 满足阈值的行的j类别的得分
            cls_boxes = boxes[inds, j * 4:(j + 1) * 4]
            cls_dets = np.hstack((cls_boxes, cls_scores[:, np.newaxis])) \
                .astype(np.float32, copy=False)  # cls_dets=[xmin,ymin,xmax,ymax,scores]
            keep = nms(cls_dets, 0.3)  # nms
