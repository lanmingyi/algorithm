...
def create_app():
    ...
    from .models.db_models import Project, ProjectAccess, ProjectVersion, Namespace, ProjectTransfer, Upload, Account, \
        RemovedProject
    ...
    def check_maintenance():
        ...
        if request.method in ['POST', 'PUT', 'PATCH', 'DELETE'] and os.path.isfile(
                current_app.config['MAINTENANCE_FILE']) and all(path not in request.path for path in allowed_endpoinds):
    ...
    def get_startup_data():
        ...
        if current_user.is_authenticated and current_user.active:
            ...
            projects_count = Project.query.filter(Project.creator_id == current_user.id).filter_by(
                namespace=current_user.username).count()
    ...
    def create_project_access_request(namespace, project_name):  # noqa: E501
        ...
        access_request = AccessRequest.query.filter_by(namespace=namespace, project_id=project.id,
    ...
    def get_project_access_requests():
        ...
        access_requests = AccessRequest.query.filter(
            (AccessRequest.user_id == current_user.id) | (AccessRequest.namespace == current_user.username)).all()
    ...
    def restore_project(id):  # noqa: E501
        ...
        project = Project.query.filter(or_(and_(Project.name == rp.name, Project.namespace == rp.namespace),
                                           Project.id == rp.properties["id"])).first()
        if project:
    ...
    def retire_removed_project(id):  # noqa: E501
        ...
        rp_dir = os.path.abspath(
            os.path.join(current_app.config['LOCAL_PROJECTS'], rp.properties["storage_params"]["location"]))
