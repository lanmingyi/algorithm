...
class RegionLayer(nn.Module):
    ...
    def build_targets(self, pred_boxes, target, nH, nW):
        nB = target.size(0)
        nA = self.num_anchors
        # torch.ones(*size):返回一个张量，该张量填充标量值1，形状由变量size定义。例如：torch.ones(2,3),torch.ones(5)
        # torch.zeros(*size):返回一个张量，该张量填充标量值0，形状由变量size定义。
    ...
    def forward(self, output, target):
        ...
        # torch.linspace(start, end, steps): steps(int)-size of constructed tensor
...
#     region_layer = RegionLayer(use_cuda=True)
#     print(region_layer)
