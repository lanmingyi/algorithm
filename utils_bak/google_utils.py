...
def attempt_download(file, repo='ultralytic/yolov3'):
    ...
    if not file.exists():
        ...
        if name in assets:
            ...
            try:  # GitHub
                ...
                print(f'Downloading {url} to {file}...')
                os.system(f'curl -L {url} -to {file}')  # torch.hub.download_url_to_file(url, weights)
            finally:
...
def gdrive_download(id='16TiPfZj7htmTyhntwcZyEEAejOUxuT6m', file='tmp.zip'):
    ...
    out = "NULL" if platform.system() == "Windows" else "/dev/null"
