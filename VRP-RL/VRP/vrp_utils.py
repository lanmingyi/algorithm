...
def create_VRP_dataset(n_problems, n_cust, data_dir, seed=None, data_type='train'):
    """
...
class DataGenerator(object):
    def __init__(self, args):
        ...
        self.test_data = create_VRP_dataset(self.n_problems, args['n_cust'], './data', seed=args['random_seed'] + 1,
                                            data_type='test')
...
class Env(object):
    def __init__(self, args):
        ...
        self.input_data = tf1.placeholder(tf1.float32, shape=[None, self.n_nodes, self.input_dim])

        self.input_pnt = self.input_data[:, :, :2]
        self.demand = self.input_data[:, :, -1]
        self.batch_size = tf1.shape(self.input_pnt)[0]
    ...
    def reset(self, beam_width=1):
        ...
        #         self.input_pnt = tf1.tile(self.input_pnt, [self.beam_width,1,1])

        # demand: [batch_size * beam_width, max_time]
        # demand[i] = demand[i+batchsize]
        self.demand = tf1.tile(self.demand, [self.beam_width, 1])

        # load: [batch_size * beam_width]
        self.load = tf1.ones([self.batch_beam]) * self.capacity

        # create mask
        self.mask = tf1.zeros([self.batch_size * beam_width, self.n_nodes],
                              dtype=tf1.float32)

        # update mask -- mask if customer demand is 0 and depot
        self.mask = tf1.concat(
            [tf1.cast(tf1.equal(self.demand, 0), tf1.float32)[:, :-1], tf1.ones([self.batch_beam, 1])], 1)

        state = State(load=self.load,
                      demand=self.demand,
                      d_sat=tf1.zeros([self.batch_beam, self.n_nodes]),
                      mask=self.mask)
    ...
    def step(self, idx, beam_parent=None):
        ...
        if beam_parent is not None:
            ...
            self.demand = tf1.gather_nd(self.demand, batchedBeamIdx)
            # load:[batch_size*beam_width]
            self.load = tf1.gather_nd(self.load, batchedBeamIdx)
            # MASK:[batch_size*beam_width x sourceL]
            self.mask = tf1.gather_nd(self.mask, batchedBeamIdx)

        BatchSequence = tf1.expand_dims(tf1.cast(tf1.range(self.batch_beam), tf1.int64), 1)
        batched_idx = tf1.concat([BatchSequence, idx], 1)

        # how much the demand is satisfied
        d_sat = tf1.minimum(tf1.gather_nd(self.demand, batched_idx), self.load)

        # update the demand
        d_scatter = tf1.scatter_nd(batched_idx, d_sat, tf1.cast(tf1.shape(self.demand), tf1.int64))
        self.demand = tf1.subtract(self.demand, d_scatter)

        # update load
        self.load -= d_sat

        # refill the truck -- idx: [10,9,10] -> load_flag: [1 0 1]
        load_flag = tf1.squeeze(tf1.cast(tf1.equal(idx, self.n_cust), tf1.float32), 1)
        self.load = tf1.multiply(self.load, 1 - load_flag) + load_flag * self.capacity

        # mask for customers with zero demand
        self.mask = tf1.concat(
            [tf1.cast(tf1.equal(self.demand, 0), tf1.float32)[:, :-1], tf1.zeros([self.batch_beam, 1])], 1)

        # mask if load= 0 
        # mask if in depot and there is still a demand

        self.mask += tf1.concat([tf1.tile(tf1.expand_dims(tf1.cast(tf1.equal(self.load, 0),
                                                                   tf1.float32), 1), [1, self.n_cust]),
                                 tf1.expand_dims(
                                     tf1.multiply(tf1.cast(tf1.greater(tf1.reduce_sum(self.demand, 1), 0), tf1.float32),
                                                  tf1.squeeze(tf1.cast(tf1.equal(idx, self.n_cust), tf1.float32))), 1)],
                                1)
