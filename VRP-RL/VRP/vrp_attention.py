...
class AttentionVRPActor(object):
    ...
    def __init__(self, dim, use_tanh=False, C=10, _name='Attention', _scope=''):
        ...
        with tf1.variable_scope(_scope + _name):
            # self.v: is a variable with shape [1 x dim]
            # self.v = tf1.get_variable('v', [1, dim], initializer=tf1.contrib.layers.xavier_initializer())
            self.v = tf1.get_variable('v', [1, dim], initializer=tf1.keras.initializers.glorot_normal())
            self.v = tf1.expand_dims(self.v, 2)

        self.emb_d = tf1.layers.Conv1D(dim, 1, _scope=_scope + _name + '/emb_d')  # conv1d
        self.emb_ld = tf1.layers.Conv1D(dim, 1, _scope=_scope + _name + '/emb_ld')  # conv1d_2

        self.project_d = tf1.layers.Conv1D(dim, 1, _scope=_scope + _name + '/proj_d')  # conv1d_1
        self.project_ld = tf1.layers.Conv1D(dim, 1, _scope=_scope + _name + '/proj_ld')  # conv1d_3
        self.project_query = tf1.layers.Dense(dim, _scope=_scope + _name + '/proj_q')  #
        self.project_ref = tf1.layers.Conv1D(dim, 1, _scope=_scope + _name + '/proj_ref')  # conv1d_4

        self.C = C  # tanh exploration parameter
        self.tanh = tf1.nn.tanh
    ...
    def __call__(self, query, ref, env):
        ...
        expanded_q = tf1.tile(tf1.expand_dims(q, 1), [1, max_time, 1])

        # v_view:[batch_size x dim x 1]
        v_view = tf1.tile(self.v, [tf1.shape(e)[0], 1, 1])

        # u : [batch_size x max_time x dim] * [batch_size x dim x 1] = 
        #       [batch_size x max_time]
        u = tf1.squeeze(tf1.matmul(self.tanh(expanded_q + e + d + ld), v_view), 2)
...
class AttentionVRPCritic(object):
    ...
    def __init__(self, dim, use_tanh=False, C=10, _name='Attention', _scope=''):
        ...
        self.project_query = tf1.layers.Dense(dim, _scope=_scope + _name + '/proj_q')  #
        self.project_ref = tf1.layers.Conv1D(dim, 1, _scope=_scope + _name + '/proj_e')  # conv1d_2
    ...
    def __call__(self, query, ref, env):
        ...
        # u : [batch_size x max_time x dim] * [batch_size x dim x 1] = 
        #       [batch_size x max_time]
        u = tf1.squeeze(tf1.matmul(self.tanh(expanded_q + e + d), v_view), 2)
