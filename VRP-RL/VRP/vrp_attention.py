...
class AttentionVRPActor(object):
    ...
    def __init__(self, dim, use_tanh=False, C=10,_name='Attention',_scope=''):
        ...
        with tf.variable_scope(_scope+_name):
            ...
            # self.v = tf.get_variable('v',[1,dim],initializer=tf.contrib.layers.xavier_initializer())
            self.v = tf.get_variable('v',[1,dim],initializer=tf.keras.initializers.glorot_normal())
