	...
	if (rawflag == 1 || flag == 1)
	{
		for (i = 0; i < height; i++)
		{
			for (j = 0; j < width; j++)
			{
				...
				//显示原图像
				pDC->SetPixel(j, i, RGB(gray, gray, gray));  // 显示图像
			}
		}
	}
...
void CMFCApplication1View::OnFileOpen()
{
	...
	CFileDialog MyFDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY, NULL, NULL);  // dialog的类实例

	MyFDlg.m_ofn.lpstrFilter = "(*.lst;*.raw;*.bmp)\0*.lst;*.raw;*.bmp\0";  // 只显示扩展名是lst和raw和bmp的文件
	MyFDlg.m_ofn.lpstrInitialDir = "D:\\LMY_Code\\ImproMLearning\\bb";  // 初始路径
...
{
	// Get the full name of image including directory
	char fullName[120];
	...
	if (rawflag == 1 || flag == 1)
	{
		fread(&width, sizeof(int), 1, fpImg);  // 读取图像宽度
		fread(&height, sizeof(int), 1, fpImg);
		if (image)
			delete image;

		image = new BYTE[width * height];  // 给image赋空间
...
void CMFCApplication1View::histCompute(BYTE*image, int width, int height)
{
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++)
		{
			gray = image[i * width + j];  // 存放（j，i）这一点的灰度级
			hist[gray]++;
...
void CMFCApplication1View::hisEqualiz(BYTE* image, int w, int h, BYTE* outImg)
{
	...
	// TODO: 浮点运算
	cons = 255. / his[255];
	OutputDebugStringA(typeid(cons).name());
	...
	for (i = 0; i < height; i++)
	{
		for (j = 0; j < width; j++)
		{
			if (i == 0 || j == 0 || i == height - 1 || j == width - 1)
				outImg[i * width + j] = 0;  // 3x3的卷积核，第一行、第一列、最后一行、最后一列不做卷积计算将输出图像在这一点的值赋为0
			else
			{
				for (m = -1; m < 2; m++)
					for (n = -1; n < 2; n++)
						block[(m + 1) * 3 + n + 1] = image[(i + m) * width + j + n];  // 对滤波点取出3x3邻域放入block
				value = convolution(smth, block);  // 卷积运算
...
void CMFCApplication1View::erosion(BYTE* image, int w, int h, BYTE* outImg)
{
	int rept;
	//腐蚀
	memcpy(outImg, image, sizeof(BYTE) * width * height);  // image复制到outImg
	...
	for (rept = 0; rept < 3; rept++) // 多次腐蚀
	{
		for (i = 1; i < h - 1; i++)
		{
			for (j = 1; j < w - 1; j++)
			{
				// 图形点(gray=255)在其3x3邻域只要有若干个背景点则该点设为背景点(0)。
