...
def draw_bbox_with_counting(image, image_index, trackers, show_box=True, show_label=True):
    ...
    for object_id, object_info in enumerate(trackers):
        # print(object_id, object_info['start_frame'])
        # print(image_index)
        # print('*****image_index********' + str(image_index))
        # print('*****start_frame******' + str(object_info['start_frame']))
        # print('********len(bboxes)********' + str(len(object_info['bboxes'])))
        if image_index >= object_info['start_frame'] and \
            ...
            # print('*****image_index**********' + str(image_index))
            # print('*****start_frame**********' + str(object_info['start_frame']))
            # print('*****object_info_index****' + str(object_info_index))
...
def counting_horizontal(center_now, center_last, line_position, line_start, line_end):
    ...
    if center_now[1] >= line_start and center_now[1] <= line_end and center_last[1] >= line_start and center_last[1] <= line_end:
