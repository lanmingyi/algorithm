...
class MaxPoolStride1(nn.Module):
    ...
    def forward(self, x):
        # F.pad(input, pad, mode='constant', value=0), input:tensor, pad:tuple, value: default 0
        # F.max_pool2d(input, kernel_size, stride, padding, dilation, ceil_mode)
        # Applies a 2D max pooling over an input signal composed of several input planes.
        # input(N,C,H,W),  kernel_size(kH,kW): the size of window to take a max over
...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        stride = self.stride
        assert(x.data.dim() == 4)  # 判断assert后的"=="两边是否相等，如果相等就继续运算，为假就报错
        B = x.data.size(0)
        C = x.data.size(1)
        H = x.data.size(2)
        W = x.data.size(3)
...
class Darknet(nn.Module):
    def getLossLayers(self):
        ...
        for m in self.models:
            # 判断m是不是RegionLayer或YoloLayer类型，是返回True，不是返回False
    ...
    def __init__(self, cfgfile, use_cuda=True):
        ...
        self.models = self.create_network(self.blocks)  # merge conv, bn, leaky，合并
    ...
    def create_network(self, blocks):
        models = nn.ModuleList()  # Holds submodules in a list. 保存子模块在列表中
        ...
        for block in blocks:
            ...
            if block['type'] == 'net':
                ...
                model = nn.Sequential()
                if batch_normalize:
                    # nn.Module.add_module(name: str, module: Optional[Module]) chiled module to be added to the module
    ...
    def load_binfile(self, weightfile):
        ...
        # torch.from_numpy(): create a tensor from numpy.ndarray
        # numpy.concatenate(): Join a sequence of arrays along an existing axis. 沿现有轴连接数组序列
