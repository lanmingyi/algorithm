...
def cv2image(cvimg):
    ...
    font = ImageFont.truetype("openpose/data/simhei.ttf", 30, encoding="utf-8")
    # font = ImageFont.truetype("pose/openpose/data/simhei.ttf", 30, encoding="utf-8")
...
def draw_detections_time(image, dets, color=(0, 255, 0)):
    # font = ImageFont.truetype("openpose/data/simhei.ttf", 16)
    # ==================ft===================
    counting_ft = PutChineseText("openpose/data/simhei.ttf")
    text_ft_size = 12
...
def draw_counting_time(image, polylines, counter_output):
    ...
    out_image = image
    # ========imagefont===============
    font = ImageFont.truetype("openpose/data/simhei.ttf", 22)
...
def draw_working_time_rtsp(image, TMCounter, end_work_flag):
    label_end = ""
    global label_during
    # font = ImageFont.truetype("openpose/data/simhei.ttf", 42)
    font = ImageFont.truetype("openpose/data/simhei.ttf", 42)
    image = Image.fromarray(image)
    draw = ImageDraw.Draw(image)
...
def draw_time_panel_rtsp(image_cpu, TMCounter, polylines, PLCounter, end_work_flag, min_time, max_time):
    ...
    # head
    # occupancy_ft = PutChineseText("openpose/data/simhei.ttf") #data/kaiti.ttf
    occupancy_ft = PutChineseText("openpose/data/simhei.ttf")  # data/kaiti.ttf
    text_ft_size = 21
...
def draw_occupancy_counting(image, polygons, counter_output, alarm_person_num):
    ...
    for polygon in polygons:
        ...
        if id == polygons_ids[0]:
            occupancy_ft = PutChineseText("openpose/data/simhei.ttf")
...
def draw_occupancy_time(image, polygons, counter_output):
    ...
    for polygon in polygons:
        ...
        # ================imagefont=======================
        # font = ImageFont.truetype("openpose/data/simhei.ttf", 20)
        # image = Image.fromarray(image)
        # draw = ImageDraw.Draw(image)
