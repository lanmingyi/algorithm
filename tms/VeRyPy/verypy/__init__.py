    ...
    for algo_name in names:
        ...
        if algo_name in algo_name_aliases:
            ...
            if algo_name in ["ps", "all", "classical"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.parallel_savings import get_ps_algorithm
                algos.append(("ps",) + get_ps_algorithm())
            if algo_name in ["ps2o", "all"]:
                # "ps2o":"Robbins and Turner (1979) CAWLIP savings algorithm",
                from tms.VeRyPy.verypy.classic_heuristics.cawlip_savings import get_ps2o_algorithm
            ...
            if algo_name in ["gpl", "all", "classical"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.gaskell_savings import get_gs_algorithm
                algos.append(("gpl",) + get_gs_algorithm())
            if algo_name in ["ss", "all", "classical"]:
                # "ss":"Webb (1964) sequential savings algorithm"
                from tms.VeRyPy.verypy.classic_heuristics.sequential_savings import get_ss_algorithm
            ...
            if algo_name in ["gps", "all", "classical"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.paessens_savings import get_gps_algorithm
            ...
            if algo_name in ["ims", "all", "classical"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.suppression_savings import get_ims_algorithm
            ...
            if algo_name in ["si", "all"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.cheapest_insertion import get_si_algorithm
            ...
            if algo_name in ["mj", "all", "classical"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.mole_jameson_insertion import get_mj_algorithm
                algos.append(("mj",) + get_mj_algorithm())
            if algo_name in ["pi", "all"]:
                # "pi":"van Breedam (1995) parallel insertion heuristic"),
                from tms.VeRyPy.verypy.classic_heuristics.cheapest_insertion import get_pi_algorithm
                algos.append(("pi",) + get_pi_algorithm())
            if algo_name in ["mbsa", "all", "classical"]:
                if not has_gurobi:
                    print("WARNING: [mbsa/DV89-MM] heuristic is not available (gurobipy is not installed).",
                          file=sys.stderr)
                else:
                    # "mbsa":"Desrochers and Verhoog (1989) matching based savings algorithm"
                    from tms.VeRyPy.verypy.classic_heuristics.matchingvrp import get_mm_algorithm
            ...
            if algo_name in ["rfcs", "all", "classical"]:
                ...
                from tms.VeRyPy.verypy.classic_heuristics.rfcs import get_rfcs_algorithm
                algos.append(("rfcs",) + get_rfcs_algorithm())
            if algo_name in ["gap", "all", "classical"]:
                if not has_gurobi:
                    print("WARNING: [gap/FJ81-GAP] heuristic is not available (gurobipy is not installed).",
                          file=sys.stderr)
                else:
                    # "gap":"Fisher & Jaikumar (1981) generalized assignment problem heuristic"
                    from tms.VeRyPy.verypy.classic_heuristics.gapvrp import get_gap_algorithm
                    algos.append(("gap",) + get_gap_algorithm())
            if algo_name in ["ptl", "all", "classical"]:
                if not has_gurobi:
                    print("WARNING: [ptl/FR76-1PTL] heuristic is not available (gurobipy is not installed).",
                          file=sys.stderr)
                else:
                    # "ptl":"Foster and Ryan (1976) Petal algorithm"
                    from tms.VeRyPy.verypy.classic_heuristics.petalvrp import get_ptl_algorithm
                    algos.append(("ptl",) + get_ptl_algorithm())
            if algo_name in ["lr3o", "all", "classical"]:
                # "lr3o":"Stewart and Golden (1982) LR3OPT heuristic"
                from tms.VeRyPy.verypy.classic_heuristics.lr3opt import get_lr3opt_algorithm
