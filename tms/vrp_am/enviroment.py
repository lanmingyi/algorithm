...
class VRPproblem:
    ...
    def __init__(self, input):
        ...
        # Coordinates of depot + other nodes 仓库坐标+其他节点坐标
    ...
    def all_finished(self):
        print('self.i.numpy() >= self.demand.shape[-1] and tf.reduce_all(tf.cast(self.visited, tf.bool))',
              self.i.numpy() >= self.demand.shape[-1] and tf.reduce_all(tf.cast(self.visited, tf.bool)))
