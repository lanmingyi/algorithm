...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def split_heads(self, tensor, batch_size):
        ...
        Splits last dimension of a tensor into (num_heads, head_depth). 将张量的最后一个维度拆分为(num_heads, head_depth)。
        Then we transpose it as (batch_size, num_heads, ..., head_depth) so that we can use broadcast
        然后我们把它转置为(batch_size, num_heads, ..., head_depth)，这样我们就可以使用广播
        """
    ...
    # 将第一个参数q作为输入，并将k, v作为参数，因此输入形状=q.shape
    def call(self, q, k, v, mask=None):
        # shape of q: (batch_size, seq_len_q, d_q).  q的形状:(batch_size, seq_len_q, d_q)
