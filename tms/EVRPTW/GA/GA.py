            ...
            if station_used.count(node[i][j]) > 1:
                for s in station:
                    if s not in station_used:
                        ...
                        node[i][j] = s
                        break
# print('station_used', station_used)
cus_data = data[data['type'] == 2]
# print("cus_data", data[data['type'] == 2])
# print('cus_data.index', cus_data.index)
...
for i in Node:
    ...
    # final_data = final_data.append(data.loc[i])
    print('data.loc[i]', data.loc[i])
    final_data = pd.concat([final_data, data.loc[i]], axis=0)
    print('final_data', final_data)
...
for i in all_Node:
    # print('final_all_data', final_all_data)
    # print('data.loc[i]', data.loc[i])
    # final_all_data = final_all_data.append(data.ix[i])
    # final_all_data = final_all_data.append(data.loc[i])
    final_all_data = pd.concat([final_all_data, data.loc[i]], axis=0)
