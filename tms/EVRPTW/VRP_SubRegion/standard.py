...
for c in range(Group_num):
    for i in range(cla):
        ...
        for j in data.index:
            ...
            each = [trans_code, vehicle_type, dist_seq, distribute_lea_tm, distribute_arr_tm, distance, trans_cost,
                    charge_cost, wait_cost, fixed_use_cost, total_cost, charge_cnt]
            final_data = final_data.append([each])
final_data.columns = ['trans_code', 'vehicle_type', 'dist_seq', 'distribute_lea_tm', 'distribute_arr_tm', 'distance',
