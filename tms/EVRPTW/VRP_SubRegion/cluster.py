...
def line(data, slope):
    ...
    for i in range(len(slope)):
        if i == 0:
            ...
            for j in data.index:
                if data.loc[j, 'lng'] < 0 and data.loc[j, 'lat'] > 0:
                    if float(data.loc[j, 'lat']) / float(data.loc[j, 'lng']) >= slope[i] and float(
                            data.loc[j, 'lat']) / float(data.loc[j, 'lng']) < slope[i - 1]:
                        data.loc[j, 'clustering'] = cluster_name[i]
    for j in data.index:
        if data.loc[j, 'lng'] < 0 and data.loc[j, 'lat'] > 0:
...
def group(data, m):
    ...
    for name, group in data.groupby('clustering'):
        ...
        c_m = pd.concat([c_m, group[group['type'] == 3]], ignore_index=True)
        print('c_m', c_m)
