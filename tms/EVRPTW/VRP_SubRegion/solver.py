...
for h in node[1:n - 1]:
    m.addConstr(
        quicksum(x[h, i] for i in node[1:n] if i != h) - quicksum(x[j, h] for j in node[0:n - 1] if h != j) == 0)
...
for i in node[1:s + 1]:
    for j in node[1:n]:
        if i != j:
            m.addConstr(
                ArrTime[i] + (Time[i, j] + SwapTime) * x[i, j] - (DueTime[0] + SwapTime) * (1 - x[i, j]) - ArrTime[
                    j] <= 0)
...
for i in node[s + 1:n - 1]:
    for j in node[1:n]:
        if i != j:
            m.addConstr(
                RemainBattery[j] - RemainBattery[i] + Consume * Distance[i, j] * x[i, j] - Battery * (1 - x[i, j]) <= 0)
for j in node[1:n]:
        ...
        if start != i:
            if (solution[start, i] > 0.5):
                if i not in solved_point and (round(sol_time[i]) >= round(sol_time[start] + 30 + Time[start, i])):
                    ...
                    return printTour(solution, route, i, totalDistance, demand, vol_demand, sol_time, solved_point,
                                     point)
