...
class Encoder(nn.Module):
    ...
    其中Sublayer(x) 表示Sub-layer对输入x做的映射，为了确保连接，
    所有的sub-layers和embedding layer输出的维数都相同 dim=512
    """
...
class Attention(nn.Module):
    """A generic attention module for a decoder in seq2seq
        seq2seq中用于解码器的通用注意力模块

        Conv1d和Conv2d的区别：2d先横着扫再竖着扫，1d只能竖着扫
    """
    ...
    def forward(self, query, ref):
        ...
        Args: 
            query: is the hidden state of the decoder at the current time step. batch x dim
                为解码器在当前时间步长的隐藏状态。第x批dim
            ref: the set of hidden states from the encoder. 
                sourceL x batch x hidden_dim
                来自编码器的隐藏状态的集合。sourceL x batch x hidden dim
        """
        # ref is now [batch_size x hidden_dim x sourceL]
        ref = ref.permute(1, 2, 0)  # 将tensor的维度换位。permute可以同时操作tensor的若干维度，transpose只能同时作用于tensor的两个维度；
...
class Decoder(nn.Module):
    ...
    第一个是Masked multi-head self-attention，也是计算输入的self-attention，
    但是因为是生成过程，因此在时刻i的时候，大于i的时刻都没有结果，只有小于i的时刻有结果，因此需要做Mask
    第二个sub-layer是全连接网络，与Encoder相同
    第三个sub-layer是对encoder的输入进行attention计算。

    同时Decoder中的self-attention层需要进行修改，因为只能获取到当前时刻之前的输入，因此只对时刻t
    之前的时刻输入进行attention计算，这也称为Mask操作。
    ...
    def apply_mask_to_logits(self, step, logits, mask, prev_idxs):
        ...
        if prev_idxs is not None:
            ...
            prev_idxs.data] = 1
            logits[maskk] = -np.inf  # np.inf 表示+∞，是没有确切的数值的,类型为浮点型
        return logits, maskk
...
class PointerNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        # torch.nn.Parameter()将一个不可训练的tensor转换成可以训练的类型parameter，并将这个parameter绑定到nn.Module里面。
