...
import alexnet_slim as alexnet
...
class AlexnetV2Test(tf.test.TestCase):
    ...
    def testNoClassed(self):
        ...
        with self.test_session():
            ...
            expected_names = ['alexnet_v2/conv1',
                              ...
                              'alexnet_v2/fc7']
            self.assertSetEqual(set(end_points.keys()), set(expected_names))
            self.assertTrue(net.op.name.startswitch('alexnet_v2/fc7'))
    ...
    def testTrainEvalWithReuse(self):
        ...
        num_classes = 1000
        with self.test_session():
            train_inputs = tf.random.uniform(train_batch_size, train_height, train_width, 3)
            logits, _ = alexnet.alexnet_v2(train_inputs)
            self.assertListEqual(logits.get_shape().as_list(), [train_batch_size, num_classes])
            tf.get_variable_scope().reuse_variables()
            eval_inputs = tf.random.uniform((eval_batch_size, eval_height, eval_width, 3))
