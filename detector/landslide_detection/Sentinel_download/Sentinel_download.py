...
if geom == 'point':
    ...
    if sys.platform.startswith('linux') or sys.platform.startswith('darwin'):
        query_geom = 'footprint:\\"Intersects(POLYGON(({lonmin} {latmin}, {lonmax} {latmin}, {lonmax} {latmax}, {lonmin} {latmax},{lonmin} {latmin})))\\"'.format(
...
if options.MaxRecords > 100:
    ...
    for i in range(int(requests_needed)):
        if (i + 1) * 100 > options.MaxRecords:
            request_list.append('%s %s %s "%s%s&rows=%d&start=%d"' % (
                wg, auth, search_output, url_search, query, options.MaxRecords % 100, i * 100))
        else:
            request_list.append(
                '%s %s %s "%s%s&rows=%d&start=%d"' % (wg, auth, search_output, url_search, query, 100, i * 100))
else:
            ...
            # download only one tile, file by file.
                ...
                if unzipped_tile_exists or options.no_download:
                    ...
                    if granule is None:
                        ...
                        print("############################################### header")
                        commande_wget = '%s %s %s%s "%s"' % (
                            wg, auth, wg_opt, product_dir_name + '/' + xml, url_header + "/" + value)
                        print(commande_wget)
                        os.system(commande_wget)
                        ...
                        commande_wget = '%s %s %s%s "%s"' % (
                            wg, auth, wg_opt, product_dir_name + '/' + "INSPIRE.xml", url_inspire + "/" + value)

                        print(commande_wget)
                        os.system(commande_wget)
                        while os.path.getsize(
                                product_dir_name + '/' + "INSPIRE.xml") == 0:  # in case of "bad gateway error"
                            os.system(commande_wget)

                        # download manifest.safe
                        url_manifest = link.replace(value, "Nodes('%s')/Nodes('manifest.safe')/" % (filename))
                        commande_wget = '%s %s %s%s "%s"' % (
                            wg, auth, wg_opt, product_dir_name + '/' + "manifest.safe", url_manifest + "/" + value)
                        print(commande_wget)
                        os.system(commande_wget)
                        while os.path.getsize(
                                product_dir_name + '/' + "manifest.safe") == 0:  # in case of "bad gateway error"
                            os.system(commande_wget)
