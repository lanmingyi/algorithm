...
def get_dataloaders(input_size=(420, 420), batch_size=2, test=None):
    ...
    pnghigh_img_lists = np.array(
        list(map(lambda x: os.path.join(pnghigh_img_dir, x), sorted(os.listdir(pnghigh_img_dir)))))
    pnghigh_ann_lists = np.array(
        list(map(lambda x: os.path.join(pnghigh_ann_dir, x), sorted(os.listdir(pnghigh_ann_dir)))))

    pnglow_img_lists = np.array(
        list(map(lambda x: os.path.join(pnglow_img_dir, x), sorted(os.listdir(pnglow_img_dir)))))
    pnglow_ann_lists = np.array(
        list(map(lambda x: os.path.join(pnglow_ann_dir, x), sorted(os.listdir(pnglow_ann_dir)))))

    pngother_img_lists = np.array(
        list(map(lambda x: os.path.join(pngother_img_dir, x), sorted(os.listdir(pngother_img_dir)))))
    pngother_ann_lists = np.array(
        list(map(lambda x: os.path.join(pngother_ann_dir, x), sorted(os.listdir(pngother_ann_dir)))))
    ...
    if test is 'all_test':
        np_tst_img = np.hstack(
            (pnghigh_img_lists[tst_high], np.hstack((pnglow_img_lists[tst_low], pngother_img_lists[tst_other]))))
        np_tst_ann = np.hstack(
            (pnghigh_ann_lists[tst_high], np.hstack((pnglow_ann_lists[tst_low], pngother_ann_lists[tst_other]))))
