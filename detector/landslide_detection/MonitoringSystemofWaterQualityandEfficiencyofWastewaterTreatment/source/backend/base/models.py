...
class ReadingType(models.Model):
    ...
    min_value = models.FloatField(null=True,
                                  blank=True)  # (Şimdilik null, sonra null = False olacak) Decimal Field daha mantıklı olabilir.
...
class Reading(models.Model):
    ...
    added_by = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True,
                                 blank=True)  # null olma olayı hoşuma gitmedi, başka çözüm gerekebilir
    unique_row_id = models.PositiveBigIntegerField(null=False)

    reading_value = models.FloatField(null=True,
                                      blank=True)  # Bunlardan sadece biri null olabilir, üstüne düşünmek lazım
