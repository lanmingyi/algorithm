...
def downloadMostRecent():
    ...
    while lastFileName != fileName:
        try:
            urlretrieve(getDownloadURL(downloadString) + fileName,
                        os.path.join(getCurrentDirectory(), 'h5Files', fileName + '.h5'))
        except:
...
def readHDF5Variables(numFiles):
    ...
    extent = getRasterExtent(
        gdal.Open(os.path.join(getCurrentDirectory(), 'referenceFile', 'reference.TIF'), gdalconst.GA_ReadOnly))
    extent[4] = xsize
    extent[5] = -ysize
    print('')
    percent = 0
    ...
    for file in h5Files:
        ...
        gpmhdf.close()
        np.savetxt(os.path.join(getCurrentDirectory(), 'Sum24', 'Files', file[-(fileNameLength + 3):-3] + '.txt'),
                   precipitationCal, delimiter=',')
        np.savetxt(os.path.join(getCurrentDirectory(), 'Sum48', 'Files', file[-(fileNameLength + 3):-3] + '.txt'),
                   precipitationCal, delimiter=',')
        np.savetxt(os.path.join(getCurrentDirectory(), 'Sum72', 'Files', file[-(fileNameLength + 3):-3] + '.txt'),
                   precipitationCal, delimiter=',')
        np.savetxt(os.path.join(getCurrentDirectory(), 'max16', 'Files', file[-(fileNameLength + 3):-3] + '.txt'),
                   precipitationCal, delimiter=',')
        try:
            dailySum(precipitationCal, file, extent)
        except:
            np.savetxt(os.path.join(getCurrentDirectory(), 'DailySums', timeStr[-15:-7] + '.txt'), precipitationCal,
                       delimiter=',')
...
def sum24(newFile, extent):
    ...
    lastFile = loadtxt(os.path.join(getCurrentDirectory(), 'Sum24', 'Files', historicFiles[0]), dtype='float',
                       delimiter=',')
    ...
    array2raster(
        os.path.join(getCurrentDirectory(), 'Sum24', 'Tiffs', timeStr[-11:-7]) + '_24HourSum_' + lastTifNum + '.TIF',
        [extent[0], extent[3]], extent[4], extent[5], rotatedSum, gdalconst.GDT_Float32)
    ...
    if np.max(percentIncrease) > 1000:
        array2raster(os.path.join(getCurrentDirectory(), 'DailySums', 'Tiffs', timeStr[-11:-7]) + '.TIF',
                     [extent[0], extent[3]], extent[4], extent[5], percentIncrease, gdalconst.GDT_Float32)
...
def sum48(newFile, extent):
    ...
    lastFile = loadtxt(os.path.join(getCurrentDirectory(), 'Sum48', 'Files', historicFiles[0]), dtype='float',
                       delimiter=',')
    currentSum = currentSum - lastFile
    currentSum = currentSum + newFile
    ...
    if not tiffFiles:
        ...
        lastTifNum = str(int(lastTif[lastTif.rfind('_') + 1:lastTif.rfind('.')]) + 1)
    array2raster(
        os.path.join(getCurrentDirectory(), 'Sum48', 'Tiffs', timeStr[-11:-7]) + '_48HourSum_' + lastTifNum + '.TIF',
        [extent[0], extent[3]], extent[4], extent[5], rotatedSum, gdalconst.GDT_Float32)
    while len(tiffFiles) > 48:
        os.remove(tiffFiles[0])
...
def sum72(newFile, extent):
    ...
    lastFile = loadtxt(os.path.join(getCurrentDirectory(), 'Sum72', 'Files', historicFiles[0]), dtype='float',
                       delimiter=',')
    currentSum = currentSum - lastFile
    currentSum = currentSum + newFile
    ...
    if not tiffFiles:
        ...
        lastTifNum = str(int(lastTif[lastTif.rfind('_') + 1:lastTif.rfind('.')]) + 1)
    array2raster(
        os.path.join(getCurrentDirectory(), 'Sum72', 'Tiffs', timeStr[-11:-7]) + '_72HourSum_' + lastTifNum + '.TIF',
        [extent[0], extent[3]], extent[4], extent[5], rotatedSum, gdalconst.GDT_Float32)
    while len(tiffFiles) > 48:
        os.remove(tiffFiles[0])
