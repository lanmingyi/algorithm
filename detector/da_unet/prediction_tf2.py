...
from detector.da_unet.model_tf2 import da_u_net
...
def run(image_path):
    ...
    size = 512
    over_lap = 256
    img = tf.keras.Input(shape=[size, size, 3], batch_size=batch_size, dtype=tf.float32)
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        saver.restore(tf.train.latest_checkpoint(checkpoint_dir + '/' + ckpt_name))
    ...
    for x_batch1 in image_list:
        ...
        img = tf.keras.Input(shape=[size, size, 3], batch_size=batch_size, dtype=tf.float32)(x_batch)
        predict = np.array(tf.nn.sigmoid(da_u_net(img, is_training=False)))
